...
vector<tCoste> DijkstraInv(const GrafoP<tCoste>& G,
                        ...
                        vector<typename GrafoP<tCoste>::vertice>& P)
// Calcula los caminos de coste mínimo entre todos los vértices del
// grafo G y un destino. En el vector D de tamaño G.numVert()
// devuelve estos costes mínimos y P es un vector de tamaño
// G.numVert() tal que P[i] es el último vértice del camino
// de i al destino.
{
   ...
   const size_t n = G.numVert();
   vector<bool> S(n, false);                 // Conjunto de vértices vacío.
   vector<tCoste> D(n);                         // Costes mínimos hasta el destino.
   ...
   for (size_t i = 1; i <= n-2; i++) {
      ...
      S[w] = true;                          // Incluir vértice w en S.
      // Recalcular coste desde cada v no incluido en S jasta el destino a través de w.
      for (v = 0; v < n; v++)
         if (!S[v]) {
