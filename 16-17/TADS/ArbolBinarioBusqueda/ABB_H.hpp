...
public:
    ...
    void eliminarArbol(const T& elto);
...
private:
    ...
    Abin<T> copiar(Abin<T> &abin, nodo, Abb<T> abb);
};
...
//eliminar subarbol que comienza por elemento elto
template <typename T>
void Abb<T>::eliminarArbol(const T& elto)
{
    Abb<T> A = buscar(elto);
    if(!A.vacio())
    {
...
/*template <typename T>
Abin<T> Abb<T>::copiar(Abin<T> &abin, typename Abin<T>::nodo nodo, Abb<T> abb)
{
